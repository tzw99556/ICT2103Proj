...
class TableBuilder_Worldindata:
    ...
    def create_tables(self):
        ...
        "CREATE TABLE IF NOT EXISTS Hospital_admission (hosp_patients INT UNSIGNED, weekly_hosp_admissions INT UNSIGNED, date_id INT UNSIGNED NOT NULL, country_id TINYINT UNSIGNED, PRIMARY KEY(date_id, country_id), FOREIGN KEY (date_id) REFERENCES Date(date_id) ON DELETE CASCADE, FOREIGN KEY (country_id) REFERENCES Country(country_id) ON DELETE CASCADE);",
        "CREATE TABLE IF NOT EXISTS Accounts (username VARCHAR(45) NOT NULL UNIQUE, password VARCHAR(100) NOT NULL);"]
